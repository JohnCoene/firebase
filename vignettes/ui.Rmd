...
To demonstrate we will build an application that allows users to sign-up/sign-in. The very thing we do is place `useFirebase` in the Shiny UI, making sure to set the argument `ui` to `TRUE`. This imports the necessary dependencies., without it nothing will work.
...
ui <- fluidPage(
  useFirebase(ui = TRUE) # import dependencies
)
...
That is it for the mandatory options, you won't be able to launch the authentication method if you have not set the providers, {fireblaze} will remind you to do so. Now the next thing to do is launch the authentication. You could bind that to a button or have it launched when the app loads, all up to you. We'll go with the latter.
...
server <- function(input, output){
  ...
  output$plot <- renderPlot({
    f$req_signed_in() # require sign in
    plot(cars)
  })
}
